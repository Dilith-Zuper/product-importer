...
async function fetchAll(supabase, table, select, opts = {}) {
  const { pageSize = PAGE_DEFAULT, filters = [], onProgress, orderBy } = opts;
  const rows = [];
  let from = 0;
  while (true) {
    let q = supabase.from(table).select(select);
    for (const f of filters) q = q[f.op](...f.args);
    // Without an explicit order, PostgREST does NOT guarantee stable ordering
    // between paginated calls — same row can appear on adjacent pages or be
    // skipped entirely. Callers should pass orderBy with the table's PK to
    // make .range() pagination deterministic.
